<?php
/**
 * MyProtector Platform - Reviews List Widget
 * 
 * WordPress widget for displaying a list of reviews
 * 
 * @package MyProtector\Modules\Widgets\Widgets
 * @version 1.0.0
 */

namespace MyProtector\Modules\Widgets\Widgets;

class ReviewsListWidget extends \WP_Widget {
    /**
     * Constructor
     */
    public function __construct() {
        parent::__construct(
            'mp_reviews_list',
            __('MyProtector Reviews List', 'myprotector-platform'),
            [
                'description' => __('Display a list of reviews for a business', 'myprotector-platform'),
                'classname' => 'mp-widget-reviews-list',
            ]
        );
    }

    /**
     * Widget output
     * 
     * @param array $args
     * @param array $instance
     * @return void
     */
    public function widget($args, $instance): void {
        echo $args['before_widget'];
        
        if (!empty($instance['title'])) {
            echo $args['before_title'] . apply_filters('widget_title', $instance['title']) . $args['after_title'];
        }
        
        $business_id = $instance['business_id'] ?? 0;
        $limit = $instance['limit'] ?? 5;
        
        echo '<div class="mp-reviews-list-widget" data-business-id="' . esc_attr($business_id) . '" data-limit="' . esc_attr($limit) . '">';
        echo '<p>' . esc_html__('Reviews list will be displayed here.', 'myprotector-platform') . '</p>';
        echo '</div>';
        
        echo $args['after_widget'];
    }

    /**
     * Widget form in admin
     * 
     * @param array $instance
     * @return void
     */
    public function form($instance): void {
        $title = $instance['title'] ?? '';
        $business_id = $instance['business_id'] ?? 0;
        $limit = $instance['limit'] ?? 5;
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title:', 'myprotector-platform'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>">
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('business_id'); ?>"><?php esc_html_e('Business ID:', 'myprotector-platform'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('business_id'); ?>" name="<?php echo $this->get_field_name('business_id'); ?>" type="number" value="<?php echo esc_attr($business_id); ?>">
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('limit'); ?>"><?php esc_html_e('Number of reviews:', 'myprotector-platform'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('limit'); ?>" name="<?php echo $this->get_field_name('limit'); ?>" type="number" min="1" max="20" value="<?php echo esc_attr($limit); ?>">
        </p>
        <?php
    }

    /**
     * Update widget instance
     * 
     * @param array $new_instance
     * @param array $old_instance
     * @return array
     */
    public function update($new_instance, $old_instance): array {
        $instance = [];
        $instance['title'] = sanitize_text_field($new_instance['title'] ?? '');
        $instance['business_id'] = absint($new_instance['business_id'] ?? 0);
        $instance['limit'] = absint($new_instance['limit'] ?? 5);
        return $instance;
    }
}